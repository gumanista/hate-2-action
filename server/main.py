...
async def create_project(project: ProjectCreate, api_key: str = Depends(get_api_key)):
    with Database() as db:
        ...
        return Project(project_id=project_id, name=project.name, description=project.description, website=project.website, contact_email=project.contact_email)

@app.get("/projects", response_model=List[Project])
async def get_projects(api_key: str = Depends(get_api_key)):
    with Database() as db:
        projects = db.get_projects()
        return [Project(project_id=p[0], name=p[1], description=p[2], created_at=p[3], website=p[4], contact_email=p[5]) for p in projects]
...
async def get_project(project_id: int, api_key: str = Depends(get_api_key)):
    with Database() as db:
        ...
        return Project(project_id=project[0], name=project[1], description=project[2], created_at=project[3], website=project[4], contact_email=project[5])

@app.put("/projects/{project_id}", response_model=Project)
async def update_project(project_id: int, project: ProjectUpdate, api_key: str = Depends(get_api_key)):
    with Database() as db:
        success = db.update_project(project_id, project.name, project.description, project.website, project.contact_email)
        if not success:
            raise HTTPException(status_code=404, detail="Project not found")
        ...
        return Project(project_id=updated_project[0], name=updated_project[1], description=updated_project[2], created_at=updated_project[3], website=updated_project[4], contact_email=updated_project[5])
...
async def get_problems(api_key: str = Depends(get_api_key)):
    with Database() as db:
        ...
        return [Problem(problem_id=p[0], name=p[1], context=p[2], created_at=p[3], is_processed=p[4]) for p in problems]
...
async def get_problem(problem_id: int, api_key: str = Depends(get_api_key)):
    with Database() as db:
        ...
        return Problem(problem_id=problem[0], name=problem[1], context=problem[2], created_at=problem[3], is_processed=problem[4])
...
async def update_problem(problem_id: int, problem: ProblemUpdate, api_key: str = Depends(get_api_key)):
    with Database() as db:
        ...
        return Problem(problem_id=updated_problem[0], name=updated_problem[1], context=updated_problem[2], created_at=updated_problem[3], is_processed=updated_problem[4])
...
async def update_solution(solution_id: int, solution: SolutionUpdate, api_key: str = Depends(get_api_key)):
    with Database() as db:
        ...
        return Solution(solution_id=updated_solution[0], name=updated_solution[1], context=updated_solution[2], created_at=updated_solution[3])
...
async def update_organization(organization_id: int, organization: OrganizationUpdate, api_key: str = Depends(get_api_key)):
    with Database() as db:
